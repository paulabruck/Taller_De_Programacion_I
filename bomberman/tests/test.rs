...
use bomberman::file::{save_maze_in_file, parse_maze, read_file};
...
fn test_save_maze_in_file() {
    ...
    let result = save_maze_in_file(&maze, "maze4.txt");
...
fn test_create_objects() {
    ...
    let  _result = match create_objects(&mut file_contents, 0, 0, maze) {
        Ok(data) => data,
...
fn test_detonar_bomba_detour_location() {
    ...
    let  _game_data = match create_objects(&mut file_contents, 4, 0, maze) {
        Ok(data) => data,
        Err(_error) => return assert!(false, "Los objetos no se pudieron crear correctamente"),
    };
}
